...
var App = function() {
    ...
    $(".docs-nav a").click(function(){
		...
		if ($(document).data("ref")==undefined) {$(document).data("ref",ref);} else {var ref=$(document).data("ref");}
		console.log(ref);
		var arr=[];
		$("#articles").find("a").each(function(){
			arr.push($(this).attr("href"));
		});
		var len=arr.length-1;
		var pos=array_search(ref,arr)*1;
		if (pos==0) {
			var prev=arr[pos]; var next=arr[pos+1];
		} else {
			if (pos==len) {
				var next=arr[pos]; var prev=arr[pos-1];
			} else {
				var next=arr[pos+1]; var prev=arr[pos-1];
			}
		}
		if ($(this).hasClass("prev")) {var goto=prev;} else {var goto=next;}
		var goto=explode(".",goto)[0].substr(1);
		$.get("/engine/ajax.php?mode=getajax&form=page&item="+goto,function(data){
			$(".site-section-top h1 strong").html($(data).find("h3:first").text());
			$(".site-block .row").html(data);
			$(document).data("ref","/"+goto+".htm");
		});
		return false;
	});
