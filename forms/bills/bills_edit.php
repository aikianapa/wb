    ...
    <div class="form-group row" name="totals">
        ...
        <div class="col-3 .summ"><input type="number" name="summ" class="form-control text-right" placeholder="Сумма" readonly></div>
		  ...
		  <div class="modal-footer">
			...
			<button type="button" class="btn btn-default" onclick="{{_GET[form]}}_print()"><span class="fa fa-print"></span> Печать</button>
...
<script language="javascript">
    {{_GET[form]}}_count();
    $("#{{_GET[form]}}EditForm").delegate("[name=products] :input","change",function(){
        if ($(this).val()>"") {
    ...
    function {{_GET[form]}}_count() {
        var prods=$("#{{_GET[form]}}EditForm [name=products]");
        var totals=$("#{{_GET[form]}}EditForm [name=totals]");
    ...
    function {{_GET[form]}}_print() {
        $.ajax({
            async: false,
            type: 'POST',
            url: "/form/print/bills/_form",
            data: $("#{{_GET[form]}}EditForm").serialize(),
