...
function wb_cart() {
  ...
  $(document).delegate("form[data-wb-role=cart] .add-to-cart", "click", function() {
    ...
    var ajax = $(this).parents("form[data-wb-role=cart]").attr("data-ajax");
    if (ajax == undefined || ajax == "") {
      var ajax = "/ajax/cart/";
    }
    if ($(this).hasClass("add-to-cart")) {
      ajax += "?action=add-to-cart";
    }
    $.get(ajax, form, function(data) {
      that.trigger("add-to-cart-done", [getcookie("order_id")]);
      if ($("[data-wb-role=cart][data-wb-tpl]").length) {wb_setdata("[data-wb-role=cart][data-wb-tpl]",{});}
  ...
  $(document).on("cart-clear", function(event) {
    var ajax = "/ajax/cart&action=cart-clear";
  ...
  $(document).on("cart-item-recalc", function(event, item, flag) {
    ...
    var fld = $(this).parents("[data-wb-role=cart]").attr("data-update");
    if (fld == undefined || fld == "") {
      var fld = new Array("quant", "price");
    } else {
      var fld = explode(",", fld);
    }
    ...
    var ajax = $(this).parents("[data-wb-role=cart]").attr("data-ajax");
    if (ajax == undefined || ajax == "") {
      var ajax = "/ajax/cart/";
    }
    ...
    $("[data-wb-role=cart] .cart-item[idx=" + idx + "] .cart-item-total").html(index);
  });

  $(document).on("cart-item-remove", function(event, item, flag) {
    var idx = $(item).attr("idx");
    var ajax = $(this).parents("[data-wb-role=cart]:not(form):first").attr("data-ajax");
    if (ajax == undefined || ajax == "") {
      var ajax = "/ajax/cart/";
    }
    form = "action=cart-item-remove&index=" + idx;
    if (flag !== "noajax") {
      var diff = $.get(ajax, form);
    }
  ...
  $(document).on("cart-item-plus", function(event, item, flag) {
    var idx = $(item).attr("idx");
    var quants = $("[data-wb-role=cart] .cart-item[idx=" + idx + "] .cart-item-quant");
    var max = 1000;
    var ajax = $(this).parents("[data-wb-role=cart]").attr("data-ajax");
    if (ajax == undefined || ajax == "") {
      var ajax = "/ajax/cart/";
  ...
  $(document).on("cart-total-recalc", function(event, item, flag) {
    var total = 0;
    var lines = 0;
    $("[data-wb-role=cart]:not(form):first .cart-item").each(function() {
