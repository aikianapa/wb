...
function wbInitEnviroment()
{
    ...
    if (isset($_ENV['settings']["cache_lifetime"]) and intval($_ENV['settings']["cache_lifetime"]) > 0) {
        $_ENV["cache_lifetime"] = intval($_ENV['settings']["cache_lifetime"]);
    }
    ...
    if (!isset($_ENV["settings"]["cache_lazy_img"])) {
        $_ENV["settings"]["cache_lazy_img"] = "";
    }
...
function wbFieldBuild($param, $data = array(), $locale = array())
{
    ...
    $label = $param['label'];
    if (isset($lang["labels"]) and isset($lang["labels"][$_SESSION["lang"]]) and $lang["labels"][$_SESSION["lang"]]["name"] > "") {
        $label = $param['label'] = $lang["labels"][$_SESSION["lang"]]["name"];
    }
        ...
        case 'select':
            ...
            if (strpos($opt["from"], ":")) {
                ...
                $select->attr("placeholder", $param["label"])
                    ...
                    ->addClass($opt["class"]);
                if ($opt["fldval"] > "") {
                    $select->html("{{" . $opt["fldval"] . "}}");
                }

                if ($opt["fldkey"] > "") {
                    $select->attr("value", "{{" . $opt["fldkey"] . "}}");
                }
                ...
                switch ($type) {
                    case "tree":
                        ...
                        $tree = $tree["tree"];
                        if (isset($from[1])) {
                            $tree = wbTreeFindBranch($tree, $from[1]);
                        }
...
function wbCacheCheck()
{
    ...
    if (isset($_ENV["settings"]["cache"]) and is_array($_ENV["settings"]["cache"])) {
        ...
        // cache cleaner
        $currtime = time();
        if (!isset($_ENV["settings"]["cache_lastdrop"]) or $_ENV["settings"]["cache_lastdrop"] == "") {
            $_ENV["settings"]["cache_lastdrop"] = $currtime;
        }
...
function wbGetCache()
{
    ...
    $dir = $_ENV['dbac'] . '/' . $sub;
    $name = $dir . '/' . $cid . '.html';
    $lifetime = $_ENV['settings']['cache_lifetime'];
    if (is_file($name)) {
        if ((time() - filectime($name)) > $lifetime) {
            ...
            exec("wget -b -F -O {$name} {$_ENV['route']['hostp']}{$_ENV['route']['uri']}?update && rm wget-* &");
        }
...
function ______wbGetCacheId($file = false)
{
...
function wbGetForm($form = null, $mode = null, $engine = null)
{
    ...
    $locale = $out->wbSetFormLocale($ini);
    if ($locale !== null) {
        wbEnvData("form->{$aCall}->locale", $locale);
    }
...
function wbBr2nl($str)
{
    ...
    return preg_replace('=<br */?>=i', 'n', $str);}function wbCheckPhpCode($code)
